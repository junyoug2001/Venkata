...
def is_hidden_run(run: Any) -> bool:
    """Return True for internal/cache runs that should stay out of normal UI lists."""
    metadata = getattr(run, "metadata", None) or {}
    if not isinstance(metadata, dict):
        return False
    return bool(
        metadata.get("hidden_from_default_lists")
        or metadata.get("is_fit_cache")
        or metadata.get("cache_run")
    )
...
class ViewState:
    ...
    fit_overlay_show_labels: bool = True
    ...
    def __post_init__(self) -> None:
        ...
        self.fit_overlay_show_labels = bool(self.fit_overlay_show_labels)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ViewState":
        ...
        base_data["fit_overlay_show_labels"] = bool(base_data.get("fit_overlay_show_labels", True))
...
class ExperimentSet:
    ...
    def add_run(self, run: Run) -> None:
        ...
        if existing is not None and existing is not run:
            ...
            raise ValueError(
                f"Run ID '{run.id}' already belongs to {visibility} run "
                f"'{existing.nickname}'."
            )
    ...
    def remove_run(self, run_id: str) -> None:
        ...
        cache_ids = [
            ...
            if is_hidden_run(run) and str((run.metadata or {}).get("source_run_id", "")) == str(run_id)
        ]
        for cache_id in cache_ids:
            del self.runs[cache_id]
...
def _new_object_id(prefix: str) -> str:
    """Generate an opaque, collision-resistant identifier with a readable prefix."""
    import uuid
...
def new_run_id(prefix: str = "run") -> str:
    ...
    return _new_object_id(prefix)


def new_view_id(prefix: str = "view") -> str:
    """Generate a collision-resistant view id."""
    return _new_object_id(prefix)


def new_experiment_id(prefix: str = "exp") -> str:
    """Generate a collision-resistant experiment id."""
    return _new_object_id(prefix)
