...
def _find_run(exp: ExperimentSet, token: Optional[str], view: Optional[ViewState] = None, slot: Optional[int] = None) -> Run:
    ...
    if token:
        if token in exp.runs:
            run = exp.runs[token]
            if not is_hidden_run(run):
                return run
        matches = [run for run in exp.runs.values() if not is_hidden_run(run) and run.nickname == token]
        if len(matches) == 1:
            return matches[0]
        lower = token.lower()
        matches = [run for run in exp.runs.values() if not is_hidden_run(run) and lower in run.nickname.lower()]
        if len(matches) == 1:
            return matches[0]
...
def _render_full_view(exp: ExperimentSet, view: ViewState, args):
    run_ids = [rid for rid in view.run_ids if (run := exp.get_run(rid)) and not is_hidden_run(run)][:2]
    if not run_ids:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-peaks", nargs="+", type=float, metavar="VALUE", help="Target Raman-shift peaks for --polar-area-fit, expressed in --unit or the selected view unit.")
    parser.add_argument("--peak-window", type=float, help="Half-width around each target peak in --unit (default: equivalent of 8 cm-1).")
    parser.add_argument("--center-window", type=float, help="Allowed fitted-center deviation in --unit (default: equivalent of 2 cm-1).")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.list:
        ...
        for view in exp.views.values():
            visible_count = sum(1 for rid in view.run_ids if (run := exp.get_run(rid)) and not is_hidden_run(run))
            print(f"  {view.id}\t{view.title}\t{visible_count} run(s)")
        print("Runs:")
        for run in exp.runs.values():
    ...
    if args.polar_area_fit:
        ...
        polar_unit = normalize_spectral_unit(
            args.unit or (getattr(view, "spectral_unit", None) if view is not None else "cm-1"),
            "cm-1",
        )
        target_peaks_cm1 = [float(value) for value in unit_to_cm1(np.asarray(args.target_peaks, dtype=float), polar_unit)]
        peak_window_cm1 = 8.0 if args.peak_window is None else float(unit_to_cm1(args.peak_window, polar_unit))
        center_window_cm1 = 2.0 if args.center_window is None else float(unit_to_cm1(args.center_window, polar_unit))
        ...
        out_path, row_fits, tensor_fits = polar_area_fitting.generate_polar_area_pdf(
            ...
            colors=colors,
            cmap_name=args.cmap,
            peak_window=peak_window_cm1,
            center_window=center_window_cm1,
            normalize=not args.no_polar_normalize,
            output_unit=polar_unit,
        )
        print(out_path)
