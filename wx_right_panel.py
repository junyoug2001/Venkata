...
class ViewPanel(wx.Panel):
    ...
    def _apply_selection_from_indices(self, *, reason: str = "") -> None:
        ...
        try:
            def _update_run_plots(run: Run, ix: int, iy: int, pA: RamanPlotter2d, pB: AngularPlotter, pC: SlicePlotter):
                ...
                # 1. Get Data
                shift_cm1 = np.asarray(run.shift_cm1, dtype=float)
                shift_axis = spectral_axis_for_run(run, self.get_spectral_unit())
                ...
                if I.shape != (angles.size, shift_cm1.size):
                    if I.shape == (shift_cm1.size, angles.size):
                        I = I.T
                    else:
                        ...
                        # --- STANDARD 2D RUNS (Slicing) ---
                            ...
                            if ref_run.shift_cm1 is not None and ref_run.angle_values is not None:
                                ...
                                if (
                                    ...
                                    ov_shift = np.asarray(r_overlay.shift_cm1, dtype=float)
                                    ov_angles = np.asarray(r_overlay.angle_values, dtype=float)
                                    ov_I = np.asarray(r_overlay.intensity_2d, dtype=float)
                                    if ov_I.shape != (ov_angles.size, ov_shift.size):
                                        if ov_I.shape == (ov_shift.size, ov_angles.size):
                                            ov_I = ov_I.T
                                        else:
                                            continue
    ...
    def _draw_runs(self, runs: List[Run]) -> None:
        ...
        self.plotterA1.render(
            x0, run0.angle_values, run0.intensity_2d,
        ...
        if len(runs) > 1:
            ...
            self.plotterA2.render(
                x1, run1.angle_values, run1.intensity_2d,
