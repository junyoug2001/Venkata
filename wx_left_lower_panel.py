...
from fit_overlay import find_fit_state, get_persistent_row_fit_cache, normalize_fit_overlay_mode, overlay_data
...
class FitResultsPanel(wx.Panel):
    """Read-only summary of stored 2D map fitting results."""

    def __init__(self, parent):
        super().__init__(parent)
        self.experiment: Optional[ExperimentSet] = None
        self.run: Optional[Run] = None
        self.fit_state: Optional[Dict[str, Any]] = None
        self.source_run: Optional[Run] = None
        self.source_runs: List[Run] = []
        self.view_state: Optional[ViewState] = None

        main = wx.BoxSizer(wx.VERTICAL)
        self.info_label = wx.StaticText(self, label="No fit results selected")
        main.Add(self.info_label, 0, wx.ALL | wx.EXPAND, 6)

        mode_row = wx.BoxSizer(wx.HORIZONTAL)
        mode_row.Add(wx.StaticText(self, label="View:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.choice_mode = wx.Choice(self, choices=["Both", "Slice", "Area"])
        self.choice_mode.SetSelection(0)
        mode_row.Add(self.choice_mode, 0, wx.RIGHT, 8)
        main.Add(mode_row, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 6)

        self.list = wx.ListCtrl(self, style=wx.LC_REPORT | wx.LC_SINGLE_SEL)
        for idx, label in enumerate(["Peak", "Rule", "Center", "Gamma", "Angular params"]):
            self.list.InsertColumn(idx, label)
        self.list.SetColumnWidth(0, 95)
        self.list.SetColumnWidth(1, 80)
        self.list.SetColumnWidth(2, 70)
        self.list.SetColumnWidth(3, 60)
        self.list.SetColumnWidth(4, 180)
        main.Add(self.list, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 6)

        self.figure = Figure(figsize=(3.8, 2.4), dpi=100)
        self.canvas = FigureCanvas(self, -1, self.figure)
        self.ax = self.figure.add_subplot(111)
        main.Add(self.canvas, 1, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 6)

        self.text = wx.TextCtrl(self, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_RICH2)
        main.Add(self.text, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 6)
        self.SetSizer(main)

        self.list.Bind(wx.EVT_LIST_ITEM_SELECTED, self.on_peak_selected)
        self.choice_mode.Bind(wx.EVT_CHOICE, self.on_mode_change)
        self.clear()
    ...
    def clear(self):
        ...
        self.info_label.SetLabel("No fit results selected")
        self.list.DeleteAllItems()
        self.text.SetValue("")
        self.figure.clf()
        self.ax = self.figure.add_subplot(111)
        self.ax.text(0.5, 0.5, "No fit", ha="center", va="center", transform=self.ax.transAxes)
        self.canvas.draw_idle()

    def update_for_run(self, experiment: Optional[ExperimentSet], run: Optional[Run], view_state: Optional[ViewState] = None) -> None:
        self.clear()
        ...
        if run.run_type == RunType.FIT_PARAMS:
            ...
            params_run = run
            source_ids = [str(v) for v in run.metadata.get("source_run_ids", [])]
        else:
            state, params_run, source_ids = find_fit_state(experiment, run)
        if not state:
            self.info_label.SetLabel(f"{run.nickname}: no stored 2D fit results")
            return

        self.fit_state = state
        sources: List[Run] = []
        for rid in source_ids:
            candidate = experiment.get_run(rid)
            if candidate is not None and candidate.is_2d:
                sources.append(candidate)
        if run.is_2d and all(candidate.id != run.id for candidate in sources):
            sources.insert(0, run)
        self.source_runs = sources[:2]
        self.source_run = self.source_runs[0] if self.source_runs else None

        source_label = params_run.nickname if params_run is not None else "run metadata"
        display_unit = self._display_unit()
        fit_range = "all"
        if "x_min_limit" in state and "x_max_limit" in state:
            xmin = self._spectral_value_to_display(float(state.get("x_min_limit")))
            xmax = self._spectral_value_to_display(float(state.get("x_max_limit")))
            fit_range = f"{xmin:.6g} - {xmax:.6g} {display_unit}"
        self.info_label.SetLabel(f"{run.nickname}: fit from {source_label}, range {fit_range}")

        peaks = state.get("peaks") or []
        self._set_peak_column_label(2, f"Center ({display_unit})")
        self._set_peak_column_label(3, f"Gamma ({display_unit})")
        for idx, peak in enumerate(peaks):
            rule = str(peak.get("rule", ""))
            spec = peak.get("spec_params", {})
            ang = peak.get("ang_params", {})
            center = self._spectral_value_to_display(self._param_value(spec.get("x0")))
            gamma = self._spectral_width_to_display(self._param_value(spec.get("gamma")))
            params = ", ".join(f"{k}={self._param_value(v):.4g}" for k, v in ang.items())
            row = self.list.InsertItem(idx, str(peak.get("name") or f"Peak {idx + 1}"))
            self.list.SetItem(row, 1, rule)
            self.list.SetItem(row, 2, f"{center:.4g}")
            self.list.SetItem(row, 3, f"{gamma:.4g}")
            self.list.SetItem(row, 4, params)
        if peaks:
            self.list.Select(0)
        self._update_text()
        self._plot_peak(0)

    def _set_peak_column_label(self, col: int, text: str) -> None:
        item = wx.ListItem()
        item.SetText(text)
    ...
    def _display_unit(self) -> str:
        if self.view_state is not None:
            return normalize_spectral_unit(getattr(self.view_state, "spectral_unit", None))
        return normalize_spectral_unit(config.get("default_spectral_unit", config.get("unit", "meV")))

    def _fit_unit(self) -> str:
        if self.fit_state:
            return normalize_spectral_unit(self.fit_state.get("unit", "cm-1"), "cm-1")
        return "cm-1"

    def _spectral_value_to_cm1(self, value: float) -> float:
        if not np.isfinite(value):
            return value
        converted = unit_to_cm1(np.asarray([value], dtype=float), self._fit_unit())
        return float(converted[0])

    def _spectral_value_to_display(self, value: float) -> float:
        if not np.isfinite(value):
            return value
        converted = cm1_to_unit(np.asarray([self._spectral_value_to_cm1(value)], dtype=float), self._display_unit())
        return float(converted[0])

    def _spectral_width_to_display(self, value: float) -> float:
        if not np.isfinite(value):
            return value
        zero = self._spectral_value_to_display(0.0)
        width = self._spectral_value_to_display(abs(value))
        return abs(width - zero)

    def _param_value(self, values) -> float:
        try:
    ...
    def _update_text(self):
        if not self.fit_state:
            return
        bg_state = self.fit_state.get("bg_params", {})
        if isinstance(bg_state, list):
            bg_lines = []
            for idx, bg in enumerate(bg_state[:2]):
                label = "XX" if idx == 0 else "YX"
                bg_lines.append(
                    f"Background {label}: offset={self._param_value(bg.get('offset')):.6g}, "
                    f"slope_x={self._param_value(bg.get('slope_x')):.6g}, "
                    f"slope_theta={self._param_value(bg.get('slope_theta')):.6g}, "
                    f"amp_si={self._param_value(bg.get('amp_si')):.6g}"
                )
        else:
            bg_lines = [
                f"Background offset: {self._param_value(bg_state.get('offset')):.6g}",
                f"Background slope: {self._param_value(bg_state.get('slope')):.6g}",
            ]
        lines = [
            *bg_lines,
        ]
        mode = self.fit_state.get("si_bg_mode")
        if mode and mode != "none":
            lines.append(f"Si BG: {mode}")
        settings = (self.source_run.metadata or {}).get("angle_rotation") if self.source_run is not None else None
        if settings and settings.get("enabled"):
            lines.append(str(settings.get("summary") or analysis.angle_rotation_summary(settings)))
        self.text.SetValue("\n".join(lines))

    def on_peak_selected(self, event):
        self._plot_peak(event.GetIndex())

    def on_mode_change(self, event):
        self._plot_peak(self._selected_peak_index())

    def _selected_peak_index(self) -> int:
        idx = self.list.GetFirstSelected()
        return idx if idx != -1 else 0

    def _plot_peak(self, peak_index: int):
        mode = self.choice_mode.GetStringSelection() or "Slice"
        self.figure.clf()
        if not self.fit_state or not self.source_runs:
            self.ax = self.figure.add_subplot(111)
            self.ax.text(0.5, 0.5, "No fit", ha="center", va="center", transform=self.ax.transAxes)
            self.canvas.draw_idle()
            return
        peaks = self.fit_state.get("peaks") or []
        if not (0 <= peak_index < len(peaks)):
            self.canvas.draw_idle()
            return
        peak = peaks[peak_index]
        if mode == "Both":
            ax_slice = self.figure.add_subplot(121, projection="polar")
            ax_area = self.figure.add_subplot(122, projection="polar")
            self._plot_slice_peak(peak, ax_slice, compact=True)
            self._plot_area_peak(peak, ax_area, compact=True)
            self.ax = ax_slice
            self.figure.subplots_adjust(left=0.05, right=0.98, bottom=0.12, top=0.88, wspace=0.35)
        elif mode == "Area":
            self.ax = self.figure.add_subplot(111, projection="polar")
            self._plot_area_peak(peak, self.ax)
        else:
            self.ax = self.figure.add_subplot(111, projection="polar")
            self._plot_slice_peak(peak, self.ax)
        self.canvas.draw_idle()

    def _plot_slice_peak(self, peak: Dict[str, Any], ax=None, compact: bool = False):
        ax = ax or self.ax
        try:
            target = self._spectral_value_to_cm1(self._param_value(peak.get("spec_params", {}).get("x0")))
            display_target = self._spectral_value_to_display(self._param_value(peak.get("spec_params", {}).get("x0")))
            for idx, run in enumerate(self.source_runs):
                shift, angles, intensity = analysis.display_2d_from_run(run)
                if shift.size == 0:
                    continue
                ix = int(np.abs(shift - target).argmin())
                ...
                label_base = run.nickname
                ax.plot(theta, intensity[:, ix], marker=marker, linestyle="None", ms=3.0, color=raw_color, label=f"{label_base} raw")
                fit = overlay_data(self.experiment, run, "both") if self.experiment else None
                if fit and fit.global_matrix is not None and fit.global_matrix.shape[1] > ix:
                    ax.plot(theta, fit.global_matrix[:, ix], color=global_color, linestyle=linestyle, lw=1.15, label=f"{label_base} global")
                if fit and fit.row_matrix is not None and fit.row_matrix.shape[1] > ix:
                    ax.plot(theta, fit.row_matrix[:, ix], color=row_color, linestyle=linestyle, lw=1.15, label=f"{label_base} row")
            self._format_polar_axis(ax)
            title = f"Slice @ {display_target:.4g} {self._display_unit()}"
            ax.set_title(title, fontsize=9 if compact else None)
            ax.legend(fontsize=6 if compact else 7, loc="upper right", bbox_to_anchor=(1.25, 1.15) if compact else None)
        except Exception as exc:
            ax.text(0.5, 0.5, f"Cannot plot slice\n{exc}", ha="center", va="center", transform=ax.transAxes)

    def _plot_area_peak(self, peak: Dict[str, Any], ax=None, compact: bool = False):
        ax = ax or self.ax
        try:
            target = self._spectral_value_to_cm1(self._param_value(peak.get("spec_params", {}).get("x0")))
            row_fits = self._cached_row_fits_for_peak(peak, target)
            if not row_fits:
                ax.text(
                    0.5,
                    0.5,
                    "No saved row-fit cache.\nRun Global Fit or Validate once.",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                )
                return
            tensor = polar_area_fitting.fit_tensor_for_peak(row_fits, fit_state=self.fit_state)
            ...
            for idx, rf in enumerate(row_fits[:2]):
                ...
                ax.plot(theta_rad, rule_func(theta_dense, rf.config, *tensor.params), color=line_color, linestyle=linestyle, lw=1.15, label=f"{rf.run_label} tensor")
            self._format_polar_axis(ax)
            ax.set_title(f"{tensor.name} area fit", fontsize=9 if compact else None)
            ax.legend(fontsize=6 if compact else 7, loc="upper right", bbox_to_anchor=(1.25, 1.15) if compact else None)
        except Exception as exc:
            ax.text(0.5, 0.5, f"Cannot plot area\n{exc}", ha="center", va="center", transform=ax.transAxes)

    def _peak_index_for_state_peak(self, peak: Dict[str, Any]) -> int:
        peaks = self.fit_state.get("peaks") or [] if self.fit_state else []
        for idx, candidate in enumerate(peaks):
        ...
        for idx, candidate in enumerate(peaks):
            try:
                center = self._spectral_value_to_cm1(self._param_value(candidate.get("spec_params", {}).get("x0")))
            except Exception:
                continue
    ...
    def _cached_row_fits_for_peak(self, peak: Dict[str, Any], target_cm1: float) -> List[polar_area_fitting.RowPeakFit]:
        if not self.experiment or not self.fit_state:
            return []
        peak_idx = self._peak_index_for_state_peak(peak)
        if peak_idx < 0:
            return []

        out: List[polar_area_fitting.RowPeakFit] = []
        for run_idx, run in enumerate(self.source_runs):
            cache_run = get_persistent_row_fit_cache(self.experiment, run, self.fit_state)
            if cache_run is None:
                continue
            metadata = cache_run.metadata or {}
            headers = [str(v) for v in (metadata.get("row_fit_headers") or [])]
            rows = metadata.get("row_fit_params") or []
            rf = self._row_fit_from_cache_table(run, cache_run, peak, peak_idx, target_cm1, headers, rows, run_idx)
            if rf is not None:
                out.append(rf)
        return out

    def _row_fit_from_cache_table(
        self,
        source_run: Run,
        cache_run: Run,
        peak: Dict[str, Any],
        ...
        rows: Any,
        run_idx: int,
    ) -> Optional[polar_area_fitting.RowPeakFit]:
        ...
        def column(col: Optional[int], default: float = np.nan) -> np.ndarray:
            values = []
            for row in rows:
                try:
                    values.append(float(row[col]) if col is not None else default)
                except Exception:
                    values.append(default)
            return np.asarray(values, dtype=float)

        angles = column(angle_col)
        areas = column(area_col)
        gammas = column(gamma_col)
        heights = column(height_col)
        centers = np.full(angles.shape, float(target_cm1), dtype=float)
        return polar_area_fitting.RowPeakFit(
            target=float(target_cm1),
            name=str(peak.get("name") or f"Peak {peak_idx + 1}"),
            ...
            gammas=gammas,
            heights=heights,
            config=polar_area_fitting.infer_config(source_run, "parallel" if run_idx == 0 else "cross"),
            run_id=source_run.id,
            run_label=source_run.nickname,
        )

    def _format_polar_axis(self, ax) -> None:
...
class PreferencesPanel(wx.Panel):
    ...
    def __init__(self, parent, on_use_current_view=None):
        ...
        cosmic_default_row = wx.BoxSizer(wx.HORIZONTAL)
        self.spin_cosmic_height = wx.SpinCtrlDouble(self, min=0, max=1_000_000, inc=50, size=(86, -1))
        self.spin_cosmic_ratio = wx.SpinCtrlDouble(self, min=0, max=1_000_000, inc=0.5, size=(86, -1))
        cosmic_default_row.Add(wx.StaticText(self, label="H"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        cosmic_default_row.Add(self.spin_cosmic_height, 0, wx.RIGHT, 6)
        cosmic_default_row.Add(wx.StaticText(self, label="R"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        cosmic_default_row.Add(self.spin_cosmic_ratio, 0)
        grid.Add(wx.StaticText(self, label="Cosmic default"), 0, wx.ALIGN_CENTER_VERTICAL)
        grid.Add(cosmic_default_row, 0, wx.EXPAND)

        cosmic_exp_row = wx.BoxSizer(wx.HORIZONTAL)
        self.chk_exp_cosmic_override = wx.CheckBox(self, label="Override")
        self.spin_exp_cosmic_height = wx.SpinCtrlDouble(self, min=0, max=1_000_000, inc=50, size=(86, -1))
        self.spin_exp_cosmic_ratio = wx.SpinCtrlDouble(self, min=0, max=1_000_000, inc=0.5, size=(86, -1))
        cosmic_exp_row.Add(self.chk_exp_cosmic_override, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        cosmic_exp_row.Add(wx.StaticText(self, label="H"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        cosmic_exp_row.Add(self.spin_exp_cosmic_height, 0, wx.RIGHT, 6)
        cosmic_exp_row.Add(wx.StaticText(self, label="R"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        cosmic_exp_row.Add(self.spin_exp_cosmic_ratio, 0)
        grid.Add(wx.StaticText(self, label="Experiment cosmic"), 0, wx.ALIGN_CENTER_VERTICAL)
        grid.Add(cosmic_exp_row, 0, wx.EXPAND)
        ...
        si_row = wx.BoxSizer(wx.HORIZONTAL)
        self.file_si_bg = wx.FilePickerCtrl(
            self,
            message="Choose default Si BG JSON profile",
            wildcard="JSON files (*.json)|*.json",
            style=wx.FLP_OPEN | wx.FLP_FILE_MUST_EXIST | wx.FLP_USE_TEXTCTRL,
        )
        self.btn_clear_si_bg = wx.Button(self, label="Clear")
        si_row.Add(self.file_si_bg, 1, wx.RIGHT, 5)
        si_row.Add(self.btn_clear_si_bg, 0)
        grid.Add(wx.StaticText(self, label="Default Si BG"), 0, wx.ALIGN_CENTER_VERTICAL)
        grid.Add(si_row, 1, wx.EXPAND)
        ...
        for ctrl in (self.spin_cosmic_height, self.spin_cosmic_ratio):
            ctrl.Bind(wx.EVT_SPINCTRLDOUBLE, self.on_change)
            ctrl.Bind(wx.EVT_TEXT_ENTER, self.on_change)
        self.chk_exp_cosmic_override.Bind(wx.EVT_CHECKBOX, self.on_experiment_cosmic_change)
        for ctrl in (self.spin_exp_cosmic_height, self.spin_exp_cosmic_ratio):
            ctrl.Bind(wx.EVT_SPINCTRLDOUBLE, self.on_experiment_cosmic_change)
            ctrl.Bind(wx.EVT_TEXT_ENTER, self.on_experiment_cosmic_change)
        self.choice_cmap.Bind(wx.EVT_COMBOBOX, self.on_change)
        self.choice_cmap.Bind(wx.EVT_TEXT, self.on_change)
        self.file_si_bg.Bind(wx.EVT_FILEPICKER_CHANGED, self.on_change)
        self.btn_clear_si_bg.Bind(wx.EVT_BUTTON, self.on_clear_si_bg)
    ...
    def update_experiment(self, experiment: Optional[ExperimentSet]) -> None:
        self._experiment = experiment
        self._refresh_experiment_cosmic_controls()

    def _refresh_experiment_cosmic_controls(self) -> None:
        if not hasattr(self, "chk_exp_cosmic_override"):
            return
        self._loading = True
        try:
            enabled = self._experiment is not None
            metadata = getattr(self._experiment, "metadata", None) if self._experiment is not None else None
            cosmic = metadata.get("cosmic_detection") if isinstance(metadata, dict) else None
            has_override = isinstance(cosmic, dict)
            height = self._coerce_float(
                cosmic.get("height") if has_override else config.get("cosmic_threshold", DEFAULT_SETTINGS["cosmic_threshold"]),
                DEFAULT_SETTINGS["cosmic_threshold"],
            )
            ratio = self._coerce_float(
                cosmic.get("ratio") if has_override else config.get("cosmic_ratio", DEFAULT_SETTINGS["cosmic_ratio"]),
                DEFAULT_SETTINGS["cosmic_ratio"],
            )
            self.chk_exp_cosmic_override.SetValue(bool(enabled and has_override))
            self.spin_exp_cosmic_height.SetValue(height)
            self.spin_exp_cosmic_ratio.SetValue(ratio)
            self.chk_exp_cosmic_override.Enable(enabled)
            self.spin_exp_cosmic_height.Enable(enabled and has_override)
            self.spin_exp_cosmic_ratio.Enable(enabled and has_override)
        finally:
            self._loading = False
    ...
    def refresh_from_config(self):
        ...
        try:
            ...
            self.spin_cosmic_height.SetValue(self._coerce_float(config.get("cosmic_threshold", DEFAULT_SETTINGS["cosmic_threshold"]), DEFAULT_SETTINGS["cosmic_threshold"]))
            self.spin_cosmic_ratio.SetValue(self._coerce_float(config.get("cosmic_ratio", DEFAULT_SETTINGS["cosmic_ratio"]), DEFAULT_SETTINGS["cosmic_ratio"]))
            ...
            self.file_si_bg.SetPath(str(config.get("default_si_bg_profile_path", "") or ""))
        finally:
            self._loading = False
        self._refresh_experiment_cosmic_controls()
    ...
    def _save_values(self):
        ...
        config.set("cosmic_threshold", float(self.spin_cosmic_height.GetValue()))
        config.set("cosmic_ratio", float(self.spin_cosmic_ratio.GetValue()))
        ...
        config.set("default_si_bg_profile_path", self.file_si_bg.GetPath() or "")

    def on_change(self, event):
        self._save_values()
        obj = event.GetEventObject() if event else None
        if obj in (self.spin_cosmic_height, self.spin_cosmic_ratio) and not self.chk_exp_cosmic_override.GetValue():
            self._refresh_experiment_cosmic_controls()
        if event:
            event.Skip()

    def on_experiment_cosmic_change(self, event):
        if self._loading:
            if event:
                event.Skip()
            return
        ...
        if self.chk_exp_cosmic_override.GetValue():
            metadata["cosmic_detection"] = {
                "height": float(self.spin_exp_cosmic_height.GetValue()),
                "ratio": float(self.spin_exp_cosmic_ratio.GetValue()),
            }
        else:
            metadata.pop("cosmic_detection", None)
        self._refresh_experiment_cosmic_controls()
        if event:
            event.Skip()

    def on_clear_si_bg(self, event):
        self.file_si_bg.SetPath("")
        config.set("default_si_bg_profile_path", "")
...
class PlotConfigPanel(wx.Panel):
    def __init__(self, parent, on_reset=None, on_view_config_change=None):
        super().__init__(parent)

        self._on_reset = on_reset
        self._on_view_config_change = on_view_config_change
    ...
    def on_unit_change(self, event):
        ...
        if self.target_view:
            ...
            if xlim and xlim[0] is not None:
                self.set_x_range(xlim[0], xlim[1])
        if self._on_view_config_change:
            self._on_view_config_change()
